 ...
 *
 * <p>The converter resolves annotation type symbols and enum constant
 * {@link VarSymbol}s lazily via {@link Symtab#enterClass(ModuleSymbol, Name)}
 * and {@link ClassSymbol#complete()}; completion failures (missing
 ...
 *
 * <p>The expected element type drives a couple of source-side
...
final class IndexAnnotations {
    ...
    IndexAnnotations(Symtab syms, Names names, Types types) {
        this.syms = syms;
        this.names = names;
        this.types = types;
    }
    ...
    List<Attribute.Compound> toCompounds(java.util.List<AnnotationRef> refs, ModuleSymbol module) {
        if (refs == null || refs.isEmpty()) return List.nil();
        ListBuffer<Attribute.Compound> out = new ListBuffer<>();
        for (AnnotationRef ref : refs) {
            Attribute.Compound compound = toCompound(ref, module);
     ...
     * semantics.
     */
    Attribute.Compound toCompound(AnnotationRef ref, ModuleSymbol module) {
        if (ref == null) return null;
        ClassSymbol annType = resolveAnnotationClass(ref.jvmName(), module);
        ...
        for (Map.Entry<String, AnnotationValue> e : ref.values().entrySet()) {
            MethodSymbol element = findElement(annType, e.getKey());
            if (element == null) continue;
            Type elementType = element.getReturnType();
            Attribute attr = toAttribute(e.getValue(), elementType, module);
            if (attr == null) continue;
            pairs.add(new Pair<>(element, attr));
        }
        return new Attribute.Compound(annType.type, pairs.toList());
    }
     ...
     * declared enum type).
     */
    Attribute toAttribute(AnnotationValue value, Type expectedType, ModuleSymbol module) {
        if (value == null) return null;
        if (value instanceof AnnotationValue.Unsupported) return null;
        ...
        if (expectedType instanceof ArrayType at && !(value instanceof AnnotationValue.Arr)) {
            Attribute single = toAttribute(value, at.elemtype, module);
            if (single == null) return null;
            return new Attribute.Array(expectedType, new Attribute[]{single});
        }

        if (value instanceof AnnotationValue.Primitive p) {
            Type t = expectedType == null || expectedType.isErroneous() ? defaultPrimitiveType(p.boxed()) : expectedType;
        ...
        if (value instanceof AnnotationValue.Arr arr) {
            ...
            for (AnnotationValue elem : arr.elements()) {
                Attribute a = toAttribute(elem, elementType, module);
        ...
        if (value instanceof AnnotationValue.Nested n) {
            Attribute.Compound inner = toCompound(n.annotation(), module);
    ...
    private ClassSymbol resolveAnnotationClass(String jvmName, ModuleSymbol module) {
        if (jvmName == null || jvmName.isEmpty()) return null;
        // Source-indexed refs may carry a simple name (no '/') because
        // the indexer doesn't know the package; in that case javac will
        // create a top-level class symbol with no package, which won't
        // resolve. Skip those — the annotation is unusable for the
        // compiler anyway.
        return enterClass(module, jvmName);
    }
    ...
    private MethodSymbol findElement(ClassSymbol annType, String elementName) {
        if (annType == null || annType.members_field == null) return null;
        Name n = names.fromString(elementName);
        Iterator<Symbol> it = annType.members_field.getSymbolsByName(n).iterator();
        while (it.hasNext()) {
            Symbol s = it.next();
            if (s instanceof MethodSymbol m) return m;
        }
        return null;
    }

    private VarSymbol findEnumConstant(Type enumType, String name) {
        if (enumType == null || !(enumType.tsym instanceof ClassSymbol enumClass)) return null;
        try {
            enumClass.complete();
        } catch (CompletionFailure ignored) {
            // tolerate; the lookup below will simply find nothing
        }
        if (enumClass.members_field == null) return null;
        Name n = names.fromString(name);
        Iterator<Symbol> it = enumClass.members_field.getSymbolsByName(n).iterator();
        while (it.hasNext()) {
            Symbol s = it.next();
            if (s instanceof VarSymbol v) return v;
        }
        return null;
    }

    private Type defaultPrimitiveType(Object boxed) {
        if (boxed instanceof Boolean) return syms.booleanType;
        if (boxed instanceof Byte) return syms.byteType;
        if (boxed instanceof Short) return syms.shortType;
        if (boxed instanceof Character) return syms.charType;
        if (boxed instanceof Integer) return syms.intType;
        if (boxed instanceof Long) return syms.longType;
        if (boxed instanceof Float) return syms.floatType;
        if (boxed instanceof Double) return syms.doubleType;
